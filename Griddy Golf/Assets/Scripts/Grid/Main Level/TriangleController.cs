...
public class TriangleController : MonoBehaviour {
	...
	public BounceSound bounceSound;
	...
	void Update () {
		/*
		if (Input.GetButtonUp ("Fire2") && !textController.hasWon && tileDotsController.dotSelected) {
			if (gridLines.stopTime && !doNotCreateDot && (numOfTotalGridDots < 30) && !manipulateVertices.isDotHighlighted) { //(numOfTotalGridDots <= 30), changed for tutorial
				//Debug.Log ("GridDot");
				CreateGridDot ();
	...
	void CreateTriangle () {
		...
		col1.gameObject.AddComponent<BounceSound> ();
		col2.gameObject.AddComponent<BounceSound> ();
		col3.gameObject.AddComponent<BounceSound> ();
		
		BounceSound bounce1 = col1.gameObject.GetComponent<BounceSound> ();
		BounceSound bounce2 = col2.gameObject.GetComponent<BounceSound> ();
		BounceSound bounce3 = col3.gameObject.GetComponent<BounceSound> ();
	...
	public void RecreateTriangle (int tempNum) {
		...
		//if (lr1.name.CompareTo("Triangle Line ") && lr2.name.CompareTo
		...
		if (numOfSelectedLines == 0) {
			...
			while (GameObject.Find ("Line Collider " + k.ToString ()) != null) {
				if (GameObject.Find ("Line Collider " + k.ToString ()).name.Equals (nameOfLine)) {
					if (k%3 == 0) {
						GameObject.Find ("Line Collider " + (k-1).ToString ()).GetComponent<AnglesText> ().startBlinking = true;
						GameObject.Find ("Line Collider " + (k-2).ToString ()).GetComponent<AnglesText> ().startBlinking = true;
						break;
					} else if ((k+1)%3 == 0) {
						GameObject.Find ("Line Collider " + (k-1).ToString ()).GetComponent<AnglesText> ().startBlinking = true;
						GameObject.Find ("Line Collider " + (k+1).ToString ()).GetComponent<AnglesText> ().startBlinking = true;
			...
			while (GameObject.Find ("Line Collider " + k.ToString ()) != null) {
				if (GameObject.Find ("Line Collider " + k.ToString ()).name.Equals (nameOfLine)) {
					...
					GameObject.Find ("Line Collider " + k.ToString ()).GetComponent<AnglesText> ().startBlinking = false;
					GameObject.Find ("Line Collider " + k.ToString ()).GetComponent<AnglesText> ().lineRend.material.color = Color.magenta;
					GameObject.Find ("Line Collider " + k.ToString ()).GetComponent<AnglesText> ().blinkingTimer = 0f;
					...
					if (k%3 == 0) {
						if (firstSelectedLine.name.Equals ("Line Collider " + (k-1).ToString ())) {
							...
							thirdSelectedLine.GetComponent<AnglesText> ().startBlinking = false;
							thirdSelectedLine.GetComponent<AnglesText> ().lineRend.material.color = Color.magenta;
							thirdSelectedLine.GetComponent<AnglesText> ().blinkingTimer = 0f;
							foundLine = true;
							break;
			...
			if (foundLine) {
				...
				float temp = thirdSelectedLine.GetComponent<AnglesText> ().angleOfLine;
